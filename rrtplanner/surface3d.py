...
class RRT3D(RRT):
    ...
    def plan(self, xstart: np.ndarray, xgoal: np.ndarray):
        ...
        while i < self.n:
            ...
            path = self.compute_path(vnearest, xnew, xs, zs)

            if (path is not None) and tuple(xnew) not in sampled and j != self.n:
                sampled.add(tuple(xnew))
                vbest = vnearest

                # check least-cost path to xnew
                cbest = self.cost(vcosts, xs, vbest, xnew)
                vnear = self.within(xs, xnew, self.r_rewire)
                for vn in vnear:
                    xn = xs[vn]
                    cn = self.cost(vcosts, xs, vn, xnew)
                    if cn < cbest:
                        possible_path = self.compute_path(vn, xnew, xs, zs)
                ...
                for vn in vnear:
                    xn = xs[vn]
                    cn = vcosts[vn]
                    cmaybe = self.cost(vcosts, xs, vn, xnew)
                    if cmaybe < cn:
                        path = self.compute_path(vn, xnew, xs, zs)
                        if path is not None:
                            parent = parents[vn]
    ...
    def compute_path(self, v, x, xs, zs, n=60):
        x0 = xs[v]
        x1 = x
        z0 = np.squeeze(zs[v])

        pathx = np.linspace(x0, x1, num=n)
        path_terrainz = self.zinterp(pathx)
        pathz = optimal_path(
            pathx,
            path_terrainz,
            z0,
...
if __name__ == "__main__":
    ...
    rrts = RRT3D(og, 2500, 50, zterrain)
    ...
    # plot surface
    mlab.mesh(X, Y, zterrain, colormap="BuGn")
